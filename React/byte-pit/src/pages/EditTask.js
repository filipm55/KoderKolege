...
const EditTask = () => {
    ...
    const {data:task, error} = useFetch(`http://localhost:8080/problems/${id}`);
    ...
    if(task){
        ...
        setVisibility(task.isPrivate);
        setTaskCategory(task.problemType)
        setExamplePairs(Object.entries(task.inputOutputExamples).map(([input, output]) => ({ input, output })));    }
        return () => {
